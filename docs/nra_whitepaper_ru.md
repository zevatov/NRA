...
**Бенчмарк: Время Холодного Старта из Облака (50 ГБ Датасет, 1 Gbps сеть)**

| Метод | Время загрузки | Время распаковки | Ожидание старта (TTFB) | Глобальный Shuffle? |
|-------|----------------|------------------|------------------------|---------------------|
| **Классика (Tar.gz)** | ~409 сек | ~100 сек | ❌ **509.0 секунд** | Нет |
| **Raw Disk (S3 -> SSD)** | ~409 сек | 0 сек | ❌ **409.0 секунд** | ✅ Да |
| **WebDataset** | 0 сек (Стриминг) | 0 сек | ✅ **0.5 секунд** | ❌ **Нет (Локальный буфер)** |
| **NRA v4.5** | 0 сек (Стриминг) | 0 сек | ✅ **0.08 секунд** | ✅ **Да (O(1))** |
...
Благодаря этой архитектуре, вы можете запустить обучение **одной строчкой кода**:

```python
import nra

# Подключаемся к архиву прямо в облаке
...
> **Анализ:** Красная линия (`Tar.gz`) демонстрирует "мертвое время" — нейросеть просто простаивает первые **50 секунд**, пока стабильный гигабитный интернет (1 Gbps) скачивает архив из S3 и процессор его распаковывает. Функция потерь не меняется. 
> В то же время голубая линия (`NRA v4.5`) начинает падать **сразу же (мгновенно)**. Dataloader тянет картинки поштучно прямо из облака, и градиентный спуск стартует без задержек.
> 
