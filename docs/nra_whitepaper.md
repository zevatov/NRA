...
Thanks to this architecture, you can start training with **a single line of code**:

```python
import nra

# Connect to the archive directly in the cloud
dataset = nra.BetaArchive("https://s3.aws.com/my-bucket/dataset-50GB.nra")

# PyTorch instantly fetches files directly from the cloud via network (O(1))!
image_bytes = dataset.read_file("image_49999.jpg")
```
